...
class OpenAIClient(BaseLLMClient):
    ...
    def chat(self, messages: List[Dict[str, Any]], is_multimodal: bool = False, json_mode: bool = False, **kwargs) -> str:
        ...
        params = {
            ...
            "timeout": kwargs.get("timeout", int(os.getenv("OPENAI_REQUEST_TIMEOUT", "600"))),
        }
...
class MiniMaxClient(OpenAIClient):
    """MiniMax M-series client through the OpenAI-compatible chat API.

    MiniMax M2.x models are reasoning models.  ``reasoning_split`` controls
    where the chain-of-thought appears:

    * ``True``  → thinking goes to ``reasoning_details``; ``content`` is clean.
    * ``False`` → thinking is embedded as ``<think>…</think>`` tags *inside*
      ``content``, which breaks ``json_mode`` parsing.

    We therefore **always** set ``reasoning_split=True`` and use
    ``max_completion_tokens`` (not ``max_tokens``) so that the token budget
    covers both reasoning and the actual JSON output.
    """

    _THINK_RE = None  # lazily compiled

    def __init__(self, model_name: str):
        super().__init__(
            model_name,
            api_key=os.getenv("MINIMAX_API_KEY") or os.getenv("OPENAI_API_KEY"),
            base_url=os.getenv("MINIMAX_BASE_URL") or os.getenv("OPENAI_BASE_URL")
        )

    @classmethod
    def _strip_think_tags(cls, text: str) -> str:
        """Remove ``<think>…</think>`` blocks that MiniMax may embed."""
        import re
        if cls._THINK_RE is None:
            cls._THINK_RE = re.compile(r"<think>.*?</think>", re.DOTALL)
    ...
    def chat(self, messages: List[Dict[str, Any]], is_multimodal: bool = False, json_mode: bool = False, **kwargs) -> str:
        ...
        # MiniMax API only supports ``max_tokens``, not ``max_completion_tokens``.
        # With reasoning_split=True the reasoning tokens go to a separate field
        # but still count against max_tokens, so we scale up to leave room for
        # both the chain-of-thought and the actual JSON output.
        raw_max = kwargs.get("max_tokens", 16384)
        if extra_body.get("reasoning_split"):
            raw_max = max(raw_max, 32768)
        params = {
            "model": self.model_name,
            "messages": processed_messages,
            "temperature": kwargs.get("temperature", 0.1),
...
class MoonshotClient(OpenAIClient):
    ...
    Kimi K2 reasoning models share the ``max_tokens`` budget between
    ``reasoning_content`` and ``content``.  We therefore:

    1. Increase ``max_tokens`` to 32 768 so reasoning does not crowd out
       the actual JSON output.
    2. Skip ``response_format: json_object`` – Kimi interprets this as
       "return a single JSON **object**", which conflicts with prompts
       requesting a JSON **array** and can cause only one record to be
       returned.  The prompt already asks for JSON; the parser handles
    ...
    def chat(self, messages: List[Dict[str, Any]], is_multimodal: bool = False,
        ...
        params = {
            ...
            "timeout": kwargs.get("timeout",
                                  int(os.getenv("OPENAI_REQUEST_TIMEOUT", "600"))),
        }

        if "extra_body" in kwargs:
            params["extra_body"] = kwargs["extra_body"]

        # Deliberately omit response_format even when json_mode=True.
        # Kimi K2 returns a single JSON object (not array) under json_object
        # mode, which loses records.  The prompt + parser handle JSON fine.
...
class MiMoClient(OpenAIClient):
    ...
    def __init__(self, model_name: str):
        super().__init__(
            ...
            base_url=os.getenv("MIMO_BASE_URL") or "https://token-plan-sgp.xiaomimimo.com/v1"
        )
    ...
    def chat(self, messages: List[Dict[str, Any]], is_multimodal: bool = False, json_mode: bool = False, **kwargs) -> str:
        ...
        params = {
            ...
            "messages": processed_messages,
            "temperature": kwargs.get("temperature", 0.1),
            "max_completion_tokens": kwargs.get("max_completion_tokens", kwargs.get("max_tokens", 4096)),
            "timeout": kwargs.get("timeout", int(os.getenv("OPENAI_REQUEST_TIMEOUT", "600"))),
...
def build_client(provider: str, model_name: Optional[str] = None) -> BaseLLMClient:
    ...
    provider_map = {
        ...
        "minimax": (MiniMaxClient, "MiniMax-M2.7"),
