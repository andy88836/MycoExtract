"""
Legacy enhanced extraction pipeline wrapper.

Current production runs use `src.extractors.paper_level_extractor` through
`scripts/run_all_papers_full_extraction.py`. The older block/chunk extraction
paths in this module are kept for backward compatibility and should be treated
as legacy unless a caller explicitly disables paper-level aggregation.
...
# FigureExtractor: only used when enable_figure_extraction=True (off by default)
...
class EnhancedExtractionPipeline:
    ...
    Production path: `paper_level_extractor` with paper-level aggregation.
    Legacy path: older text/table block extraction used only when
    `use_paper_level_aggregation=False`.
    ...
    def __init__(
        ...
        glm47_client=None,                      # 历史参数名：第三文本模型客户端
        glm46v_client=None,                     # 历史参数名：表格视觉模型客户端
        third_text_client=None,                 # 第三文本模型客户端（推荐新参数名）
        vision_table_client=None,               # 表格视觉模型客户端（推荐新参数名）
        # Paper-level Aggregation选项（新）
        use_paper_level_aggregation: bool = False,  # 启用paper-level aggregation
        aggregation_client=None,                     # Aggregation Agent teacher客户端
        # Table-image ablation
        disable_table_image: bool = False,           # 关闭table-image分支（消融实验用）
        ...
        # v8 paper-level aggregation can run entirely from the dedicated
        # student/vision/teacher clients below. The generic text/multimodal
        # clients are only required for legacy extraction modes.
        uses_dedicated_extractor = use_paper_level_aggregation or use_multi_model_voting
        if not uses_dedicated_extractor and (not self.text_client or not self.multimodal_client):
            raise ValueError("Must provide either llm_client or both text_client and multimodal_client")

        if third_text_client is not None:
            glm47_client = third_text_client
        if vision_table_client is not None:
            glm46v_client = vision_table_client
        ...
        # Paper-level Aggregation模式 - 使用2或3个文本模型提取+配置的teacher模型聚合
        if use_paper_level_aggregation:
            # 验证所需的客户端都已提供（第三文本模型可选；参数名保留glm47_client以兼容旧调用）
            ...
            if not all(required_clients):
                raise ValueError(
                    ...
                    "The third text client is optional (can be None for 2-student mode)."
                )
            ...
            self.paper_level_extractor = create_paper_level_extractor(
                kimi_client=kimi_client,
                deepseek_client=deepseek_client,
                glm47_client=glm47_client,  # legacy parameter name; may be MiniMax or another configured text model
                glm46v_client=glm46v_client or multimodal_client,
                aggregation_client=aggregation_client,
                text_prompt_path=self.text_prompt_path,
                table_prompt_path=self.table_prompt_path,
                figure_prompt_path=self.figure_prompt_path,
                disable_table_image=disable_table_image,
            )

            third_model_name = getattr(glm47_client, "model_name", None) if glm47_client else None
            vision_model = glm46v_client or multimodal_client
            vision_model_name = getattr(vision_model, "model_name", None) or "configured vision model"
            teacher_model_name = getattr(aggregation_client, "model_name", None) or "configured teacher model"

            # 根据第三文本模型是否提供显示不同的日志
            if glm47_client:
                logger.info("✓ Paper-level aggregation extractor initialized (3-student mode)")
                logger.info(f"  - Student models: Kimi, DeepSeek, {third_model_name}")
            else:
                logger.info("✓ Paper-level aggregation extractor initialized (2-student mode)")
                logger.info("  - Student models: Kimi, DeepSeek")
            logger.info(f"  - Multimodal model: {vision_model_name}")
            logger.info(f"  - Teacher model: {teacher_model_name} (Aggregation Agent)")
    ...
    def _process_paper(self, paper_dir: str) -> Dict[str, Any]:
        ...
        if self.use_paper_level_aggregation and self.paper_level_extractor:
            ...
            if all_records:
                all_records = self._filter_primary_dataset_records(all_records)
            ...
            if all_records:
                ...
                all_records = self._validate_row_level_records(all_records)
    ...
    def _filter_primary_dataset_records(self, records: List[Dict]) -> List[Dict]:
        """Exclude review-derived rows from the primary dataset output."""
        filtered = []
        excluded = 0
        for record in records:
            ...
            document_type = record.get('Document_Type') or record.get('document_type') or ''
            qc_status = record.get('QC_Status') or record.get('qc_status') or ''
            if (
                str(source_type).lower() == 'review'
                or str(measurement_type).lower() == 'review_background'
                or str(document_type).lower() == 'review'
                or str(qc_status).lower() == 'exclude_review_article'
            ):
        ...
        if excluded:
            logger.info(f"  Excluded {excluded} review-derived records from primary dataset")
        return filtered

    def _validate_row_level_records(self, records: List[Dict]) -> List[Dict]:
        """Attach row-level error flags for downstream review."""
        try:
            from src.utils.row_level_validator import RowLevelValidator
            return RowLevelValidator.validate_batch(records)
        except Exception as exc:
            logger.warning(f"Row-level validation skipped: {exc}")
            return records
    ...
    def _clean_duplicate_fields(self, records: List[Dict]) -> List[Dict]:
        ...
        STANDARD_FIELDS = {
            ...
            "sequence_availability", "source_type", "matrix", "reaction_matrix",
            "application_matrix", "cofactor", "cofactors", "buffer",
            "notes", "evidence_text", "source_section", "source_table_id",
            "source_channel", "source_model", "raw_record_index", "locked_candidate",
    ...
    def _merge_enzyme_info_within_paper(self, records: List[Dict]) -> List[Dict]:
        ...
        # v8.2: only low-risk fields may be propagated by enzyme name.
        # Do not propagate full names, genes, database IDs, organisms, or
        # optimum conditions across measurement contexts.
        BASIC_INFO_FIELDS = [
            'enzyme_type',
            'ec_number',
        ]
            ...
            for record in group_records:
                ...
                for field, value in basic_template.items():
                    if record.get(field) in (None, '', [], {}):
                        ...
                        propagated_note = f"propagated_from=same_paper_enzyme_group:{field}"
                        record['notes'] = f"{notes}; {propagated_note}" if notes else propagated_note
                        filled_count += 1
    ...
    def _save_final_results(self, all_results: Dict, failed_papers: Dict, output_dir: str):
        ...
        if unknown_substances:
            ...
            try:
                response = self.text_client.chat(messages=[{"role": "user", "content": judge_prompt}], temperature=0.1, max_tokens=500, task="substrate_judge")
