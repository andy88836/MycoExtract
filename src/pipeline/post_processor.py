...
MEASUREMENT_TYPE_VALUES = {
    ...
    "degradation",
    "biotransformation",
    "application_matrix_degradation",
    "product_identification",
    "optimum_condition",
    "enzyme_activity_assay",
    "stability",
    "inhibition",
    "crude_fraction_activity",
    "putative_activity",
    "review_background",
    "ambiguous_conflicting_source",
}

CONDITION_SCOPE_VALUES = {
    "kinetic_assay",
    "degradation_assay",
    "optimum_condition",
    "activity_assay",
...
GENERIC_ACTIVITY_SUBSTRATES = {
    "abts", "guaiacol", "dmp", "2,6-dimethoxyphenol", "catechol",
    "syringaldazine", "veratryl alcohol", "rbbr", "reactive black 5",
    "methylene blue", "dppd"
}


def _has_value(value: Any) -> bool:
    """Return True for values that carry extracted information."""
    return value not in (None, "", [], {}, "Unknown", "unknown", "N/A", "n/a")


def _norm_text(value: Any) -> Optional[str]:
    """Normalize text for comparison keys while preserving nulls."""
    if not _has_value(value):
        return None
    if isinstance(value, list):
        value = ";".join(str(v) for v in value if _has_value(v))
    return str(value).strip().lower()


def _norm_number(value: Any) -> Optional[str]:
    """Normalize numeric-ish values for conservative merge keys."""
    if not _has_value(value):
        return None
    try:
...
def infer_measurement_type(record: Dict[str, Any]) -> str:
    """Infer measurement_type for legacy records without changing existing fields."""
    existing = record.get("measurement_type")
    if existing in MEASUREMENT_TYPE_VALUES:
        return existing
    ...
    if str(source_type).lower() == "review":
        return "review_background"

    notes = str(record.get("notes") or "").lower()
    if "conflict" in notes or "contradict" in notes:
        return "ambiguous_conflicting_source"

    substrate = str(record.get("substrate") or "").lower()
    has_kinetic = any(_has_value(record.get(f)) for f in ["Km_value", "Vmax_value", "kcat_value", "kcat_Km_value"])
    has_degradation = _has_value(record.get("degradation_efficiency"))
    has_optimum = _has_value(record.get("optimal_ph")) or _has_value(record.get("optimal_temperature_value"))
    has_stability = _has_value(record.get("thermal_stability")) or _has_value(record.get("thermal_stability_time"))

    if substrate and any(s in substrate for s in GENERIC_ACTIVITY_SUBSTRATES):
        return "enzyme_activity_assay"
    if has_kinetic:
        return "kinetic"
    if has_degradation:
        matrix = _norm_text(record.get("matrix") or record.get("application_matrix") or record.get("reaction_matrix"))
        if matrix and matrix not in {"buffer", "solution", "model solution", "aqueous solution"}:
            return "application_matrix_degradation"
        return "degradation"
    if has_optimum:
        return "optimum_condition"
    if has_stability:
        return "stability"
    return "putative_activity"


def infer_condition_scope(record: Dict[str, Any], measurement_type: Optional[str] = None) -> str:
    """Infer condition_scope from measurement_type for legacy records."""
    existing = record.get("condition_scope")
    if existing in CONDITION_SCOPE_VALUES:
        return existing

    mt = measurement_type or infer_measurement_type(record)
    if mt == "kinetic":
        return "kinetic_assay"
    if mt in {"degradation", "biotransformation"}:
        return "degradation_assay"
    if mt == "application_matrix_degradation":
        return "application_matrix"
    if mt == "enzyme_activity_assay":
        return "activity_assay"
    if mt == "optimum_condition":
        return "optimum_condition"
    if mt == "stability":
        return "stability_assay"
    return "unknown"


def apply_context_condition_mapping(record: Dict[str, Any]) -> Dict[str, Any]:
    """
    Fill context-specific condition fields from legacy fields when safe.

    Legacy fields are preserved for backwards compatibility.
    """
    measurement_type = infer_measurement_type(record)
    condition_scope = infer_condition_scope(record, measurement_type)
    record.setdefault("measurement_type", measurement_type)
    record.setdefault("condition_scope", condition_scope)

    def fill(field: str, legacy_field: str):
        if not _has_value(record.get(field)) and _has_value(record.get(legacy_field)):
            record[field] = record.get(legacy_field)

    if measurement_type == "kinetic" or condition_scope == "kinetic_assay":
        fill("kinetic_temperature_value", "temperature_value")
        fill("kinetic_temperature_unit", "temperature_unit")
        fill("kinetic_ph", "ph")
        fill("kinetic_time_value", "reaction_time_value")
        fill("kinetic_time_unit", "reaction_time_unit")
    elif measurement_type in {"degradation", "biotransformation", "application_matrix_degradation"} or condition_scope in {"degradation_assay", "application_matrix"}:
        fill("degradation_temperature_value", "temperature_value")
        fill("degradation_temperature_unit", "temperature_unit")
        fill("degradation_ph", "ph")
        fill("degradation_time_value", "reaction_time_value")
        fill("degradation_time_unit", "reaction_time_unit")
    elif measurement_type == "enzyme_activity_assay" or condition_scope == "activity_assay":
        fill("activity_assay_temperature_value", "temperature_value")
        fill("activity_assay_temperature_unit", "temperature_unit")
        fill("activity_assay_ph", "ph")
        fill("activity_assay_time_value", "reaction_time_value")
        fill("activity_assay_time_unit", "reaction_time_unit")

    if measurement_type == "optimum_condition" or condition_scope == "optimum_condition":
        fill("optimum_temperature_value", "optimal_temperature_value")
        fill("optimum_temperature_unit", "optimal_temperature_unit")
        fill("optimum_ph", "optimal_ph")

    has_legacy_condition = any(_has_value(record.get(f)) for f in ["temperature_value", "ph", "reaction_time_value"])
    if condition_scope == "unknown" and has_legacy_condition:
        record["human_review_required"] = True
        flags = record.get("error_flags") or []
        if "unit_context_ambiguity" not in flags:
            flags.append("unit_context_ambiguity")
...
FULL_SCHEMA = {
    ...
    "degradation_time_value": None,  # Time value for degradation (prompt-facing)
    "degradation_time_unit": None,   # Time unit (min, h, etc.)
    "reaction_time_value": None,     # Canonical time field (post-processed from degradation_time_*)
    "reaction_time_unit": None,      # Canonical time unit
    "products": None,              # Reaction products
    "mediator_name": None,
    "mediator_concentration": None,
    "mediator_concentration_unit": None,
    ...
    # === Measurement Context Attribution (v8.2) ===
    "measurement_type": None,
    "measurement_context_id": None,
    "condition_scope": None,
    "reported_enzyme_name": None,
    "canonical_enzyme_name": None,
    "enrichment_status": None,
    "human_review_required": None,
    "error_flags": None,

    # === Context-specific Conditions (v8.2) ===
    "kinetic_temperature_value": None,
    "kinetic_temperature_unit": None,
    "kinetic_ph": None,
    "kinetic_time_value": None,
    "kinetic_time_unit": None,
    "degradation_temperature_value": None,
    "degradation_temperature_unit": None,
    "degradation_ph": None,
    "degradation_time_value": None,
    "degradation_time_unit": None,
    "activity_assay_temperature_value": None,
    "activity_assay_temperature_unit": None,
    "activity_assay_ph": None,
    "activity_assay_time_value": None,
    "activity_assay_time_unit": None,
    "optimum_temperature_value": None,
    "optimum_temperature_unit": None,
    "optimum_temperature_range": None,
    "optimum_ph": None,
    "optimum_ph_range": None,
    "optimum_condition_target": None,
    "kinetic_unit_multiplier": None,
    "kinetic_unit_source_text": None,
}
...
def normalize_record_schema(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    record = apply_context_condition_mapping(record.copy())
    try:
        from src.utils.table_multiplier import apply_kinetic_unit_multiplier
        record = apply_kinetic_unit_multiplier(record)
    except Exception as exc:
        logger.debug(f"Table multiplier normalization skipped: {exc}")
    normalized = {}
...
class RecordMerger:
    ...
    The algorithm:
    1. Group records by measurement context identity
    2. Within each group, check numeric compatibility
    ...
    MERGEABLE_FIELDS = [
        ...
        'products', 'mediator_name', 'mediator_concentration', 'mediator_concentration_unit',
        'temperature_value', 'temperature_unit',
        'ph', 'optimal_ph', 'optimal_temperature_value', 'optimal_temperature_unit',
        'measurement_type', 'measurement_context_id', 'condition_scope',
        'reported_enzyme_name', 'canonical_enzyme_name', 'enrichment_status',
        'human_review_required', 'error_flags',
        'kinetic_temperature_value', 'kinetic_temperature_unit', 'kinetic_ph',
        'kinetic_time_value', 'kinetic_time_unit',
        'degradation_temperature_value', 'degradation_temperature_unit', 'degradation_ph',
        'degradation_time_value', 'degradation_time_unit',
        'activity_assay_temperature_value', 'activity_assay_temperature_unit',
        'activity_assay_ph', 'activity_assay_time_value', 'activity_assay_time_unit',
        'optimum_temperature_value', 'optimum_temperature_unit', 'optimum_temperature_range',
        'optimum_ph', 'optimum_ph_range', 'optimum_condition_target',
        'kinetic_unit_multiplier', 'kinetic_unit_source_text',
        'matrix', 'reaction_matrix', 'application_matrix', 'cofactor', 'buffer',
        'notes'
    ]
    ...
    def get_entity_key(self, record: Dict) -> Tuple:
        ...
        The key is intentionally measurement-context aware. This prevents
        kinetic/degradation/activity/optimum rows for the same enzyme from
        being collapsed into one record.
        """
        record = apply_context_condition_mapping(record)
        enzyme_name = _norm_text(record.get('reported_enzyme_name')) or self.normalize_enzyme_name(record)
        ...
        measurement_type = infer_measurement_type(record)
        condition_scope = infer_condition_scope(record, measurement_type)
        matrix = _norm_text(record.get('matrix') or record.get('reaction_matrix') or record.get('application_matrix'))
        cofactor = _norm_text(record.get('cofactor') or record.get('cofactors') or record.get('buffer'))
        product = self._product_key(record)

        # Use context-specific conditions first, then legacy fields for older rows.
        ph = (
            _norm_number(record.get('kinetic_ph'))
            if measurement_type == 'kinetic'
            else _norm_number(record.get('degradation_ph'))
            if measurement_type in {'degradation', 'biotransformation', 'application_matrix_degradation'}
            else _norm_number(record.get('activity_assay_ph'))
            if measurement_type == 'enzyme_activity_assay'
            else _norm_number(record.get('optimum_ph') or record.get('optimal_ph'))
        )
        if ph is None:
            ph = _norm_number(record.get('ph'))

        temperature = (
            _norm_number(record.get('kinetic_temperature_value'))
            if measurement_type == 'kinetic'
            else _norm_number(record.get('degradation_temperature_value'))
            if measurement_type in {'degradation', 'biotransformation', 'application_matrix_degradation'}
            else _norm_number(record.get('activity_assay_temperature_value'))
            if measurement_type == 'enzyme_activity_assay'
            else _norm_number(record.get('optimum_temperature_value') or record.get('optimal_temperature_value'))
        )
        if temperature is None:
            temperature = _norm_number(record.get('temperature_value'))

        time_value = (
            _norm_number(record.get('degradation_time_value'))
            if measurement_type in {'degradation', 'biotransformation', 'application_matrix_degradation'}
            else _norm_number(record.get('kinetic_time_value'))
            if measurement_type == 'kinetic'
            else _norm_number(record.get('activity_assay_time_value'))
            if measurement_type == 'enzyme_activity_assay'
            else None
        )
        if time_value is None and measurement_type in {'degradation', 'biotransformation', 'application_matrix_degradation'}:
            time_value = _norm_number(record.get('reaction_time_value'))

        time_unit = (
            _norm_text(record.get('degradation_time_unit'))
            if measurement_type in {'degradation', 'biotransformation', 'application_matrix_degradation'}
            else _norm_text(record.get('kinetic_time_unit'))
            if measurement_type == 'kinetic'
            else _norm_text(record.get('activity_assay_time_unit'))
            if measurement_type == 'enzyme_activity_assay'
            else None
        )
        if time_unit is None and measurement_type in {'degradation', 'biotransformation', 'application_matrix_degradation'}:
            time_unit = _norm_text(record.get('reaction_time_unit'))
    ...
    def _product_key(self, record: Dict) -> Optional[str]:
        """Return a conservative product key for merge decisions."""
    ...
    def are_values_similar(self, val1: Optional[float], val2: Optional[float],
        ...
        if val1 is None and val2 is None:
            return True  # Both missing, compatible
        if val1 is None or val2 is None:
            return False  # One has data, other doesn't -- NOT similar
    ...
    def can_merge(self, record1: Dict, record2: Dict) -> bool:
        ...
        """
        record1 = apply_context_condition_mapping(record1)
        record2 = apply_context_condition_mapping(record2)

        mt1 = infer_measurement_type(record1)
        mt2 = infer_measurement_type(record2)
        scope1 = infer_condition_scope(record1, mt1)
        scope2 = infer_condition_scope(record2, mt2)

        # Never merge different measurement contexts.
        if mt1 != mt2 or scope1 != scope2:
            return False

        forbidden_pairs = {
            frozenset(("kinetic", "degradation")),
            frozenset(("kinetic", "biotransformation")),
            frozenset(("kinetic", "application_matrix_degradation")),
            frozenset(("kinetic", "optimum_condition")),
            frozenset(("enzyme_activity_assay", "degradation")),
            frozenset(("enzyme_activity_assay", "biotransformation")),
            frozenset(("enzyme_activity_assay", "application_matrix_degradation")),
            frozenset(("enzyme_activity_assay", "kinetic")),
        }
        if frozenset((mt1, mt2)) in forbidden_pairs:
            return False

        # Must have same context-aware entity key
...
def remove_silver_when_gold_exists(records: List[Dict]) -> List[Dict]:
    """
    Remove Silver-tier records when a Gold-tier record exists for the
    same enzyme + substrate + mutations + measurement_type combination.

    This handles cross-table duplicates where Table 1 (Silver) and Table 2
    (Gold) both extract the same measurement but with different quality.
    Must be called AFTER quality_tier has been assigned.
    """
    from collections import defaultdict
    groups: Dict[Tuple, List[Dict]] = defaultdict(list)
    for rec in records:
        enzyme = (rec.get('enzyme_name') or '').strip().lower()
        substrate = (rec.get('substrate') or '').strip().lower()
        mutations = (rec.get('mutations') or '').strip().lower()
        mt = infer_measurement_type(rec)
        groups[(enzyme, substrate, mutations, mt)].append(rec)

    result = []
    removed_count = 0
    for key, group in groups.items():
        gold = [r for r in group if r.get('quality_tier') == 'Gold']
        silver = [r for r in group if r.get('quality_tier') == 'Silver']
        others = [r for r in group if r.get('quality_tier') not in ('Gold', 'Silver')]

        if gold and silver:
            removed_count += len(silver)
            result.extend(gold)
    ...
    if removed_count:
        logger.info(f"Cross-table dedup: removed {removed_count} Silver records "
                    f"where Gold records exist for the same measurement")
    return result
