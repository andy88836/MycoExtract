...
class DataProcessor:
    ...
    def _merge_enzyme_info(self, records: List[Dict]) -> List[Dict]:
        ...
        # Full names, genes, database IDs, organisms, and optimum conditions
        # can cause abbreviation/enrichment cascade errors.
        BASIC_FIELDS = ['enzyme_type', 'ec_number']
            ...
            for r in group:
                for field, val in template.items():
                    if not r.get(field) or r.get(field) in ('', 'Unknown'):
                        ...
                        r['notes'] = f"{notes}; {propagated_note}" if notes else propagated_note
...
async def run_post_extraction(
    ...
    """运行后处理流水线"""
    pipeline = create_pipeline(llm_client=llm_client, config=config)
