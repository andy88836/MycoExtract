...
v9: This module is deprecated. All quality logic consolidated into quality_tier.py.
- Mycotoxin whitelist → QualityTierClassifier._is_mycotoxin_substrate()
- Non-enzymatic check → QualityTierClassifier._is_nonenzymatic()
- Enzyme identity check → QualityTierClassifier._has_enzyme_identity()
- Hard rules + tier calculation → QualityTierClassifier.classify_record()
...
class QualityConstraintFilter:
    ...
    MYCOTOXINS = {
        ...
        # Mycotoxin metabolites / hydrolysis products (still acceptable as
        # substrates because they are commonly the assayed compound when the
        # paper studies a downstream degradation step)
        'aflatoxicol', 'aflatoxin q1', 'afq1',
        'ochratoxin alpha', 'ochratoxin α',
        'fusarenon-x', 'fus-x',
        'don-3-glucoside', 'd3g',
        'zearalenone-14-glucoside', 'zen-14g',

        # Generic family names that appear in titles/abstracts
    ...
    NON_MYCOTOXIN_SUBSTRATES = {
        ...
        # Resorufin substrates (cytochrome P450 / cytochrome b5 assays)
        ...
        # Polycyclic aromatics / non-mycotoxin xenobiotics
        ...
        # Other inorganic / oxidants
        'hydrogen peroxide', 'h2o2', 'h₂o₂',
        'manganese', 'mn2+', 'mn(ii)',
        'phenol', 'benzene',

        # Glutathione / cysteine substrate panel (GST assays)
        'glutathione', 'gsh', 'gssg', 'oxidized glutathione',
        'monodehydroascorbate', 'dehydroascorbate',
        'sphingosine-1-phosphate', 's1p',

        # Reactive oxygen / nitrogen species (not actual substrates)
        'reactive oxygen species', 'ros', 'superoxide',

        # Probiotic / commercial preparations seen in OTA mitigation studies
        'synbiotic a', 'synbiotic b', 'synbiotic c',
        'bioplus 2b', 'cylactin', 'inulin',

        # Tissue / matrix concentrations (not enzyme substrates)
    ...
    def _check_sequence_availability(self, record: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        enzyme_name = str(record.get('enzyme_name') or '').lower()
        enzyme_full_name = str(record.get('enzyme_full_name') or '').lower()
    ...
    def _check_mycotoxin_substrate(self, record: Dict[str, Any]) -> Tuple[bool, str]:
        """检查底物是否是霉菌毒素"""
        substrate = str(record.get('substrate') or '').lower().strip()
        ...
        # 未识别的底物 — bypass if LLM flagged as mycotoxin (pending human review)
        is_llm_flagged = str(record.get("human_review_required", "")).lower() == "true"
        if is_llm_flagged:
            logger.info(f"LLM-flagged mycotoxin '{substrate}' - passed for human review")
            return True, f"LLM-identified mycotoxin '{substrate}' - pending human review"
        if self.strict_mode:
    ...
    def _check_detoxification(self, record: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        for prod in products:
            if isinstance(prod, dict):
                product_texts.append(str(prod.get('name') or '').lower())
