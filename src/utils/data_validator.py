...
class DataValidator:
    ...
    def validate_and_clean(cls, record: Dict[str, Any], block_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        v9: Always returns the record. Score 1 records are preserved (not deleted).
        Quality tier classification happens downstream in QualityTierClassifier.
        ...
        Returns:
            Cleaned record with confidence score, or None if record is empty
        """
        if not record:
            return None

        # 1. Calculate Confidence Score (1-3) - v5.0: Top-level field
        score = cls.calculate_confidence(record)
        record["confidence_score"] = score

        # 2. Add Block ID for traceability (optional metadata)
        if block_id:
        ...
        # v9: never reject records here — let QualityTierClassifier handle it
        if score <= 1:
            logger.info(f"Record scored {score} (preserved for tier classification): {cls._get_record_summary(record)}")
